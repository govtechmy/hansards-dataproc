...
def rename_pdf_files(paths):
    for path in paths:
        match = re.search(r"DR-(\d{2})(\d{2})(\d{4})\.pdf", path.name, re.IGNORECASE)
        if match:
...
def clean_up():
    ...
    for filename in filenames:
        shutil.copy(filename, Path.cwd() / "src_hansard" / "done" / filename.name)
...
if __name__ == "__main__":
    ...
    filenames = get_filenames_in_folder(Path.cwd() / "src_hansard" / "new")
