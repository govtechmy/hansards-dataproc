...
def mimic_table_as_plaintext(table):
    ...
    for row in table:
        ...
        for line_idx in range(max_lines):
            table_text += ' '.join(
                [cell.split('\n')[line_idx] for cell in row if line_idx < len(cell.split('\n'))]) + '\n'
    return table_text
...
def format_table(text, bold, italics, table, hansard_date):
    ...
    while start_idx >= 0 and num_text_chars_included_before_anchor < num_table_chars_before_anchor:
    ...
    while end_idx < len(text) and num_text_chars_included_after_anchor < num_table_chars_after_anchor:
        text_included_after_anchor += text[end_idx]
        num_text_chars_included_after_anchor += len(re.sub(r'\s', '', text[end_idx]))
        end_idx += 1

    candidate_text = text_included_before_anchor + text[anchor_idx] + text_included_after_anchor
    table_text_similarity_score = fuzz.ratio(re.sub(r'\s', '', candidate_text), re.sub(r'\s', '', table_text))
    ...
    for char in re.sub(r'\s', '', candidate_text):
        if char.isalnum():
            candidate_text_alphanumeric_count[char] = candidate_text_alphanumeric_count.get(char, 0) + 1
    ...
    for key in table_alphanumeric_count.keys():
        if table_alphanumeric_count[key] != candidate_text_alphanumeric_count[key]:
            print(f'Alphanumeric counts do not match: {table_alphanumeric_count[key]} vs '
                  f'{candidate_text_alphanumeric_count[key]} for key {key}')
    assert num_text_chars_included_before_anchor == num_table_chars_before_anchor and \
           num_text_chars_included_after_anchor == num_table_chars_after_anchor, \
...
def preprocess(hansard_date):
    print(hansard_date)
    year = hansard_date[-4:]
    output_dir_path = f"pretabulation/{year}/"
