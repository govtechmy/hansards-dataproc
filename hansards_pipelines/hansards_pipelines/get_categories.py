...
from hansards_pipelines.config import INPUT_PIPELINE_DIR, BASE_PATH
...
def get_categories(hansard_date, house, root_dir=INPUT_PIPELINE_DIR, file_content=None):
    ...
    if file_content:
        ...
        pdf_file = root_dir / f"{house}-{hansard_date}.pdf"
    ...
    kkdr_subcategories_non_bold_flag = False
    if len(kkdr_subcategories_non_bold) > 0 and house.upper() == "KKDR":
        if not file_content:
            ...
            kkdr_subcategories_non_bold_flag = True
    ...
    if not file_content:
        ...
        # return all data to caller for writing to s3
        return (
            long_toc,
            text,
            spaced_bold,
