...
def standardise_timestamp(timestamp):
    def mod_24(x):
        print(f"standardise_timestamp x: {x}")
        assert len(x) == 4
        if x[:2] == "24":
            x = "00" + x[2:]
        return x
...
def get_author_and_speech(text, bold, italics, house, warn="", is_pipeline=False):
    ...
    if (
        ...
        speech_bold = bold[split_idx + 1 :]
        speech_italics = italics[split_idx + 1 :]
    if author != "" and warn != "" and not is_pipeline:
...
def tabulate(
    ...
    if house.upper() != "DN" and not is_pipeline:
        with open(
            f"parsed_pdf/{house}/{year}/{sortable_date}/categories.json", "r"
        ) as f:
            categories = json.load(f)
    ...
    while row_id + 1 < num_rows:
        ...
        # run until DOA first
        if (
            "DOA" == text[row_id].strip() or "D O A" == text[row_id].strip()
        ):  # DR-29042008
            doa_seen = True
            continue
        if not doa_seen:
        ...
        if "1" not in bold[row_id]:
            ...
            if (
                ...
                if not is_pipeline:
                    with open(
                        f"warnings/{house}/level_2_following_level_1.txt", "a"
                    ) as f:
            ...
            if upper_lower_ratio(text[row_id]) > 1:
                ...
                if (
                    house.upper() == "DN"
                ):  # snippet below still applicable to DN, but removed in DR
                ...
                if current_category_probability > 0.9:
                    ...
                    current["speech_italics"] = ""
                    row_id += add_idx - 1
                    if current_category_probability < 1 and not is_pipeline:
            ...
            if re.fullmatch(
                ...
                current["speech_italics"] += italics[row_id]
                continue
            elif (hansard_date == "02112018" and house.upper() == "DR") and (
    ...
    if actual_toc != categories:
        ...
        if alphanumeric_categories != alphanumeric_actual and not is_pipeline:
    ...
    return speeches, absent_text, attended_text
